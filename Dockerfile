...
RUN go build -mod=vendor -ldflags "-X github.com/prebid/prebid-cache/version.Ver=`git describe --tags | sed 's/-.*$//'` -X github.com/prebid/prebid-cache/version.Rev=`git rev-parse --short HEAD`" .
